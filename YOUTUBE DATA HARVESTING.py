...
mydb = mysql.connector.connect(host="localhost",user="root",password="1234",database="Phonepay",auth_plugin="mysql_native_password",charset="utf8mb4")
...
if selected == "Migrate To SQL":
    ...
    # Ensure "channel_data" is defined before usage
    channel_data = pd.DataFrame()  # Initialize an empty DataFrame

    # Extract channel names from the documents in the collection
    channel_names = []
    for i in collection.find():
        channel_name = i.get("channel_data", [{}])[0].get("channel_name")
    ...
    if st.button('Migrate To SQL'):
        ...
        mydb.commit()
        sql_insert = "INSERT INTO channels (Channel_id, Channel_name, Playlist_id, Subscribers, Video_count, view_count) " \
                    "VALUES (%s, %s, %s, %s, %s, %s)"\
                    "ON DUPLICATE KEY UPDATE " \
                    "Channel_name = VALUES(Channel_name), Playlist_id = VALUES(Playlist_id), Subscribers = VALUES(Subscribers), " \
                    "Video_count = VALUES(Video_count), view_count = VALUES(view_count)"
        ...
        #playlist _data
        create_query='''create table if not exists playlist(Channel_Id varchar(100),
                                                           Channel_Name varchar(100),
                                                           Playlist_Id varchar(100) primary key,
                                                           playlist_name varchar(100))'''
        mycursor.execute(create_query)
        mydb.commit()
        sql_insert = "INSERT INTO playlist (Channel_id, Channel_name, Playlist_id, Playlist_name) " \
                    "VALUES (%s, %s, %s, %s)"\
                    "ON DUPLICATE KEY UPDATE " \
                    "Channel_Id = VALUES(Channel_Id),Channel_name = VALUES(Channel_name), Playlist_id = VALUES(Playlist_id),Playlist_name=VALUES(Playlist_name)"
        ...
        mycursor.execute(create_query)
        mydb.commit()
        sql_insert = "INSERT INTO videos (Channel_id, Channel_name, Video_id, Title, Tags, Thumbnail, Description, Published_date, Duration, Views, Likes, Comments, Favorite_count, Definition, Caption_status) " \
                    "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)" "ON DUPLICATE KEY UPDATE " "Channel_id=VALUES(Channel_id),Channel_name = VALUES(Channel_name),Video_id=VALUES(Video_id),Title = VALUES(Title), " \
        ...
        mycursor.execute(create_query)
        mydb.commit()
        sql_insert = "INSERT INTO comments (Comment_id,Video_id,Comment_text,Comment_author,Comment_posted_date,Like_count,Reply_count ) " \
...
if selected == "Analysis":   
    ...
    if questions == '1. What are the names of all the videos and their corresponding channels?':
        ...
        Q2 ='''SELECT channel_name, Video_count 
               FROM channels
        ...
        Q7 = '''select Channel_Name,view_count 
                from channels
